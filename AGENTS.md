...
- Lint/format toolchain: [Biome](https://biomejs.dev) (`@biomejs/biome`), added manually after scaffolding (not via `--toolchain`). Config in `biome.json`. Scripts: `pnpm lint` (lint only), `pnpm format` (format, write), `pnpm check` (lint + format + import sort, write). `src/routeTree.gen.ts` is excluded (generated by `tsr generate`, regenerated on every build/dev).
...
- shadcn CLI is set up (`components.json`, no `tailwind.config.js` per Tailwind v4) purely to pull `src/components/Stack.tsx` from the react-bits registry (`npx shadcn@latest add @react-bits/...`) — used for the animated card picker in the Planning Poker voting flow. That file was hand-edited after install to add an `onFrontCardClick` callback the upstream component doesn't ship. Brings in `motion` as the project's animation library (available for future use, not retrofitted onto existing CSS animations).
...
- `biome check .` currently reports 2 pre-existing findings from the CLI scaffold, left unfixed intentionally (behavioral changes, not formatting): `lint/security/noDangerouslySetInnerHtml` in `src/routes/__root.tsx` (the theme-init inline script) and `lint/suspicious/noShorthandPropertyOverrides` in `src/styles.css` (a `background` shorthand after `background-color`). Fix deliberately, don't auto-apply `--write --unsafe` blindly.
